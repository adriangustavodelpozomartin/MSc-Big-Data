...
def roc_curve(real, estimations, selClass:str, figsize=(10,10), fpr_label='False Positive Rate', tpr_label='True Positive Rate', title='Receiver operating characteristic (ROC)'):
    ...
    def plotModelDiagnosis_lineal(df, pred, target, model):
        '''
        Graficar diagnóstico del modelo para análisis de regresión.
    
        Esta función genera gráficos de diagnóstico para el análisis de regresión, 
        incluyendo histogramas de residuos y gráficos de dispersión o boxplots 
        de residuos contra cada variable en el dataframe. Para variables numéricas, 
        se crea un gráfico de dispersión de residuos contra la variable. Para 
        variables categóricas, se crea un boxplot de residuos contra las categorías.
        
        Parámetros
        ----------
        df : pd.core.frame.DataFrame
            DataFrame que contiene variables de entrada, salida y predicción.
            
        pred : str
            Nombre de la columna en `df` que contiene las predicciones del modelo.
            
        target : str
            Nombre de la columna en `df` que contiene los valores reales de salida.
            
        model : statsmodels.regression.linear_model.RegressionResultsWrapper
            Modelo de regresión ajustado previamente que se utilizará para el diagnóstico.
        
        Retorna
        -------
        None
            La función crea gráficos usando Matplotlib y no devuelve ningún valor.
        
        Ejemplos
        --------
        >>> plotModelDiagnosis(df, 'predicciones', 'salida_real', modelo_ajustado)
        '''
        # Calcular residuos
        ...
        # Calcular Cook's Distance y Leverage
        cooks_d = influence.cooks_distance[0]
        leverage = influence.hat_matrix_diag

        n = len(df)  # Número total de observaciones
        p = df.shape[1] - 2  # Número de variables (excluyendo target y pred)

        # Determinar límites de influencia
        cooks_threshold = 4 / n
        leverage_threshold = 2 * (p / n)

        # Contar puntos influyentes
        influential_cooks = np.sum(cooks_d > cooks_threshold)
        influential_leverage = np.sum(leverage > leverage_threshold)

        # Identificar outliers en los residuos
        outliers = df["residuals"].abs() > 2 * np.std(df["residuals"])  

        # Configurar el área de graficación
        fig, ax = plt.subplots(2, 4, figsize=(20, 10))  # Dos filas y cuatro columnas de gráficos

        # 1. Histograma de residuos
        sns.histplot(data=df, x="residuals", bins=30, kde=True, ax=ax[0, 0])
        ...
        sns.scatterplot(data=df, x=pred, y="residuals", ax=ax[0, 1])
        ax[0, 1].axhline(0, color='red', linestyle='--')  # Línea horizontal en cero
        ...
        sm.qqplot(df["residuals"], line='45', ax=ax[0, 2])
        ax[0, 2].set_title('Gráfico Q-Q de Residuos')

        # 4. Boxplot de residuos
        sns.boxplot(data=df, x="residuals", ax=ax[0, 3])
        ax[0, 3].set_title('Boxplot de Residuos')
        ax[0, 3].set_xlabel('Residuos')

        # 5. Gráfico de Cook's Distance
        sns.scatterplot(x=np.arange(len(cooks_d)), y=cooks_d, ax=ax[1, 0])
        ax[1, 0].axhline(cooks_threshold, color='red', linestyle='--')  # Línea de referencia
        ax[1, 0].set_title("Cook's Distance")
        ax[1, 0].set_xlabel('Observaciones')
        ax[1, 0].set_ylabel("Cook's Distance")

        # Etiquetar puntos influyentes
        for i in range(len(cooks_d)):
            if cooks_d[i] > cooks_threshold:
                ax[1, 0].annotate(i, (i, cooks_d[i]), textcoords="offset points", xytext=(0, 5), ha='center', fontsize=8)

        # 6. Gráfico de Leverage
        sns.scatterplot(x=np.arange(len(leverage)), y=leverage, ax=ax[1, 1])
        ax[1, 1].axhline(leverage_threshold, color='red', linestyle='--')  # Línea de referencia
        ax[1, 1].set_title('Leverage')
        ax[1, 1].set_xlabel('Observaciones')
        ax[1, 1].set_ylabel('Leverage')

        # Etiquetar puntos influyentes
        for i in range(len(leverage)):
            if leverage[i] > leverage_threshold:
                ax[1, 1].annotate(i, (i, leverage[i]), textcoords="offset points", xytext=(0, 5), ha='center', fontsize=8)

        # 7. Gráfico de Influencia (Combinado)
        sns.scatterplot(x=leverage, y=cooks_d, ax=ax[1, 2])
        ax[1, 2].set_title('Influence Plot (Leverage vs Cook\'s Distance)')
        ax[1, 2].set_xlabel('Leverage')
        ax[1, 2].set_ylabel("Cook's Distance")
        ax[1, 2].axhline(cooks_threshold, color='red', linestyle='--')  # Línea de referencia para Cook's Distance
        ax[1, 2].axvline(leverage_threshold, color='red', linestyle='--')  # Línea de referencia para Leverage

        # Resaltar puntos influyentes
        for i in range(len(cooks_d)):
            if cooks_d[i] > cooks_threshold and leverage[i] > leverage_threshold:
                ax[1, 2].scatter(leverage[i], cooks_d[i], color='orange', s=20, edgecolor='black', label='Influyente' if i == 0 else "")
                ax[1, 2].annotate(i, (leverage[i], cooks_d[i]), textcoords="offset points", xytext=(0, 5), ha='center', fontsize=8)

        # Espacio vacío para el texto
        ax[1, 3].axis('off')  # Deshabilitar ejes para el cuadro de texto
        ...
        \n-- Media de residuos: {np.mean(df["residuals"]):.4f}
        -- Desviación estándar de residuos: {np.std(df["residuals"]):.4f}

        \n-- Distancia de Cook: Evalúa el impacto de un 
        punto de datos en el modelo de regresión al observar 
        cambios en los coeficientes cuando se excluye dicho punto.
        -- Máxima distancia de Cook: {np.max(cooks_d):.4f}
        -- Puntos influyentes (Cook's Distance): {influential_cooks}

        \n-- Leverage: Mide cuán lejos está un punto del 
        centro de los datos y su potencial influencia en las predicciones.
        -- Media de Leverage: {np.mean(leverage):.4f}
        -- Puntos influyentes (Leverage): {influential_leverage}
        """

        # Añadir el cuadro de texto con información adicional
        ax[1, 3].text(0, 0.5, info_text, fontsize=12, va='center', ha='left', wrap=True)

        # Ajustar el diseño
        plt.tight_layout()
        plt.show()

        # Filtrar columnas excluyendo las de predicción y objetivo
        filtered_columns = [col for col in df.columns if col not in [pred, target]]

        # Configurar el área de graficación para boxplots y gráficos de dispersión
        num_plots = len(filtered_columns)
        num_cols = 4  # Número de columnas en el diseño
        num_rows = (num_plots + num_cols - 1) // num_cols  # Calcular el número de filas necesarias
        fig, ax = plt.subplots(num_rows, num_cols, figsize=(20, 5 * num_rows))  # Ajustar el tamaño
        
        # Aplanar el array de ejes para facilitar la indexación
        ax = ax.flatten()

        # 3. Boxplots y gráficos de dispersión para cada variable en el dataframe
        for i, column in enumerate(filtered_columns):
            if df[column].dtype == 'object' or df[column].dtype.name == 'category':
                # Boxplot para variables categóricas
                sns.boxplot(data=df, x=column, y="residuals", ax=ax[i])
                ax[i].set_title(f'Residuos vs. {column}')
                ax[i].set_xlabel(column)
                ax[i].set_ylabel('Residuos')
            else:
                # Gráfico de dispersión para variables numéricas
                sns.regplot(data=df, x=column, y=df["residuals"], ax=ax[i])
                ax[i].set_title(f'Residuos vs. {column}')
                ax[i].set_xlabel(column)
                ax[i].set_ylabel('Residuos')
            
            ax[i].axhline(0, color='red', linestyle='--')  # Línea horizontal en cero

        # Ocultar los ejes no utilizados si hay menos gráficos que espacios
...
def plotModelDiagnosis(df, pred, target):
    ...
    # Configurar el área de graficación
    fig, ax = plt.subplots(1, 4, figsize=(20, 5))  # Dos filas y cuatro columnas de gráficos

    # 1. Histograma de residuos
    sns.histplot(data=df, x="residuals", bins=30, kde=True, ax=ax[0])
    ax[0].set_title('Histograma de Residuos')
    ax[0].set_xlabel('Residuos')
    ax[0].set_ylabel('Frecuencia')

    # 2. Gráfico de dispersión de residuos vs. valores predichos
    sns.scatterplot(data=df, x=pred, y="residuals", ax=ax[1])
    ax[1].axhline(0, color='red', linestyle='--')  # Línea horizontal en cero
    ax[1].set_title('Residuos vs. Valores Predichos')
    ax[1].set_xlabel('Valores Predichos')
    ax[1].set_ylabel('Residuos')

    # 3. Gráfico Q-Q de residuos
    sm.qqplot(df["residuals"], line='45', ax=ax[2])
    ax[2].set_title('Gráfico Q-Q de Residuos')

    # 4. Boxplot de residuos
    sns.boxplot(data=df, x="residuals", ax=ax[3])
    ax[3].set_title('Boxplot de Residuos')
    ax[3].set_xlabel('Residuos')
...
def dotplot(scores: dict, metric: str):
    ...
    error_values = list(scores.values())

    # Crear un DataFrame de pandas para usar con seaborn
    data = {
        'Model': [],
        'Error': []
    }
    for model, errors in scores.items():
        data['Model'].extend([model] * len(errors))
    ...
    plt.figure(figsize=(10, len(model_names) * 0.5))  # Ajustar tamaño según la cantidad de modelos
    sns.boxplot(x='Error', y='Model', data=df, orient='h', palette='Set2')
    
    # Etiquetas y título
    plt.xlabel(metric)
    plt.title('Comparación de Errores de Modelos (Validación Cruzada)')
